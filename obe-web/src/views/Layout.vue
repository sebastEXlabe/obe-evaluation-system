    ...
    <el-aside :width="sidebarCollapsed ? '64px' : '220px'" class="app-side" :class="{ collapsed: sidebarCollapsed }">
      <div class="side-header">
        <template v-if="!sidebarCollapsed">
          <div class="side-sysname">AHP-OBE-CDIO</div>
          <div style="font-size:10px;color:#8fa4be;text-align:center;margin-bottom:8px">软件工程课程评价系统</div>
          <div class="side-title">{{ roleLabels[role] }}</div>
          <div class="side-name">{{ user.realName || user.username }}</div>
        </template>
        <template v-else>
          <div class="side-sysname" style="font-size:14px">A-O-C</div>
        </template>
...
.side-sysname { font-size: 16px; font-weight: bold; color: #409EFF; letter-spacing: 2px; }
